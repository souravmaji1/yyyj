...
export default function QrMobileSignupPage() {
  ...
  const [errors, setErrors] = useState({ name: "", email: "", password: "", confirmPassword: "", otp: "", kioskMachineId: "" });
        ...
        <div className="w-full max-w-md space-y-10 px-4 sm:px-6 py-2 sm:py-2 mt-[-10px] rounded-2xl shadow-2xl mx-auto">
          ...
          {isKioskInterface() && sessionId && (
            <Button
              ...
              type="button"
              onClick={() => router.push(`/auth/qr-mobile-kiosk-signin?sessionId=${encodeURIComponent(sessionId)}`)}
            >
