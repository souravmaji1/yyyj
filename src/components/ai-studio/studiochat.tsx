...
/* ----------  MAIN COMPONENT  ---------- */
export function StudioChat({ selectedImage }: StudioChatProps) {
...
useEffect(() => {
  ...
  useEffect(() => {
    if (selectedImage?.url) {
      console.log('📸 Library image selected:', selectedImage.url);
      
      // Create a File-like object for the selected image with the actual URL
      const selectedImageFile = new File([], selectedImage.name, { type: selectedImage.type });
      (selectedImageFile as any).url = selectedImage.url;
      (selectedImageFile as any).isLibrarySelected = true;
      (selectedImageFile as any).isAlreadyUploaded = true;

      // Add to existing files instead of replacing them
      setFiles(prev => {
        // Check if we already have this library image
        const exists = prev.some(f => (f as any).isLibrarySelected && (f as any).url === selectedImage.url);
        if (exists) return prev;
        
        // Add the new library image to existing files
        return [...prev, selectedImageFile];
      });
      
      // Add preview for the library image
      setPreviews(prev => {
        const exists = prev.includes(selectedImage.url);
        if (exists) return prev;
  ...
  /* ----------  FILE HANDLING  ---------- */
  // Handle file picker uploads (new files)
...
const onFilePick = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  // Simple check - just append if under limit
  if (files.length + newFiles.length > 2) {
    alert(`Max 2 files total. You have ${files.length}, trying to add ${newFiles.length} more.`);
    return;
  }

  // Simple append - just like CreateAdModal
  ...
  // Updated remove file handler
  const removeFile = (idx: number) => {
    const fileToRemove = files[idx];
    
    if ((fileToRemove as any).isLibrarySelected) {
      // Remove library image
      setFiles(prev => prev.filter((_, i) => i !== idx));
      setPreviews(prev => prev.filter(url => url !== (fileToRemove as any).url));
    } else {
      // Remove regular file
      setFiles(prev => prev.filter((_, i) => i !== idx));
...
const handleSend = async () => {
  ...
  // Separate files: library images vs file picker files
  const libraryFiles = files.filter(f => (f as any).isLibrarySelected);
  const pickerFiles = files.filter(f => !(f as any).isLibrarySelected);

  // Add library image URLs to message (don't upload)
  if (libraryFiles.length > 0) {
    const libraryUrls = libraryFiles.map(f => (f as any).url || 'library-image');
    textToSend += `\n\n(Library images: ${libraryUrls.join(', ')})`;
  }

  // Upload file picker files
  if (pickerFiles.length > 0) {
    try {
      setUploading(true);
      const documentIds = await uploadFilesToDocuments(pickerFiles);
      textToSend += `\n\n(Uploaded files: ${documentIds.join(', ')})`;
      
      // Clear only uploaded files, keep library files
      setFiles(prev => prev.filter(f => (f as any).isLibrarySelected));
      setPreviews(prev => prev.filter(url => url.startsWith('http') || url.includes('s3')));
    } catch (error) {
...
if (part.type === 'tool-imageEnhancer') {
  ...
  return (
   <div className="h-screen w-full bg-gray-950 text-gray-100 flex overflow-hidden">
        ...
        <Card className="bg-gray-950 w-full border-gray-800 shadow-2xl">
          {/* Header */}
            ...
            {!started ? (
              ...
              <div className="p-6">
                <div className="text-center mb-8">
                  ...
                  <h2 className="text-2xl font-bold text-gray-200 mb-2">Create your Image, Video, 3d-Model, Song using AI</h2>
                  <p className="text-gray-400">Generate Image , Song , Video , Model using AI. Will be displaying in Intelliverse Platform!</p>
                </div>
    ...
    <div key={i} className="relative group">
      {files[i]?.type?.startsWith('video/') ? (
        <video
          ...
          autoPlay
          playsInline
          onLoadStart={() => console.log('Video preview loading:', p)}
        ...
        <img
          ...
          alt={`preview-${i}`}
          className="w-10 h-10 object-cover rounded-xl border border-gray-700/30"
          onLoad={() => console.log('Image preview loaded:', p)}
          onError={(e) => console.log('Image preview error:', p)}
        />
      )}
      <button
